...
| 能力 | 详解 |
|------|------|
| **Data Connection** | 200+ 连接器，支持快照/增量/流式三种同步模式 |
| **Ontology Manager** | 将数据映射为业务对象，定义属性、关系、动作 |
| **权限体系** | 基于角色、分类、目的的三维细粒度控制 |
| **审计追踪** | 不可变日志，完整决策与数据变更历史 |

**关键工具**
- 03.Data Connection（企业级数据集成）
...
数据源（Snowflake/S3/ERP）
    ↓ 多源接入、版本控制
第1层：本体数据 → 业务对象定义、权限管理
    ↓ 对象属性映射
第2层：数据转换 → 清洁、丰富、派生列
    ↓ 转换结果回写
第3层：分析洞察 → 路径分析、时序分析、血缘追踪
    ↓ 分析结果展示
第4层：应用协作 → 应用UI、Action按钮、协作文档
    ↓ 用户交互与决策
第5层：AI决策 → 自主决策、LLM生成、工作流自动化
    ↓ 反馈闭环
    ↑ 决策结果回写到本体
```
